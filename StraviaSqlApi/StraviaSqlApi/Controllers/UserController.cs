...
namespace StraviaSqlApi.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public IActionResult JoinGroup(string id, string user)
        {
            if (ModelState.IsValid)
            {
        ...
        [HttpPut]
        [Route("updateGroup/newNombre")]
        public IActionResult UpdateGroup(GroupDTO group, string newNombre)
        {
            if (ModelState.IsValid)
            {
                string result = UserDAL.UpdateGroup(group, newNombre);
                if (result is "Error")
