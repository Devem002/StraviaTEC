...
namespace StraviaSqlApi.Controllers
{
    ...
    /// <returns><c>IActionResult</c> with the Challenges or an error message</returns>
        ...
        /// <param name="retoId"><c>int</c>: Challenge Id to participate on</param>
        /// <returns><c>IActionResult</c> with the participation result</returns>
        [HttpPost]
        [Route("getInChallenge/{user}/{challengeId}")]
        public IActionResult GetInChallenge(string user, string retoId, string actividadId)
        {
            if (ModelState.IsValid)
            {
                string result = ChallengeDAL.GetInChallenge(user, retoId, actividadId);
                if (result is "Error")
        ...
        public IActionResult AddChallengeActivity(string user, string challengeId, string activityId)
        {
            if (ModelState.IsValid)
            {
                string result = ChallengeDAL.AddChallengeActivity(user, challengeId, activityId);
                if (result is "Error")
                    return new JsonResult("Something went wrong while adding the challenge.") { StatusCode = 500 };
